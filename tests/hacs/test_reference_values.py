...
class TestKnownOffenders:
    ...
    KNOWN: ClassVar[dict[str, list[str]]] = {
        # Мы отдаём подписи источников из HA ("HDMI 1") вместо словаря
        # протокола (hdmi1, tv, av, …). Починка меняет allowed_values, а
        # значит и model.id — Sber перерегистрирует устройство, и
        # пользователь теряет комнату (история issue #44), поэтому это
        # отдельное решение, а не попутная правка.
        "tv": ["source"],
        # Программа уборки — это имена скоростей вентилятора из HA
        # ("Silent", "Turbo") вместо perimeter/spot/smart/random_route.
        # Статус собирается из значений standby/go_home/error, которых в
        # документации Sber нет вовсе: комментарий в vacuum_cleaner.py
        # ссылается на словарь, не соответствующий текущей спецификации.
        "vacuum_cleaner": ["vacuum_cleaner_program", "vacuum_cleaner_status"],
    }
    """Категории и функции, чьи значения сегодня расходятся со словарём Sber."""
