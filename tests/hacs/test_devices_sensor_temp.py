...
def test_different_hardware_gets_different_model_id():
    ...
    всех устройств, которые её заявляют. До 1.51 в одном config-пакете
    уезжали два устройства с одинаковым ``model.id``, но разными
    ``manufacturer`` / ``model`` / ``description`` — какое из описаний
    останется в облаке, было не определено (#63). Теперь имя железа
    входит в дайджест.

    Если тест упадёт, у пользователя с датчиками двух вендоров в
    приложении Сбера снова окажется одна модель на двоих: карточки
    покажут чужого производителя, а интерфейсы двух устройств
    склеятся.
    """
    ids = []
