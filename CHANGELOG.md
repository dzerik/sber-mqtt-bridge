...
- **Мост больше не переподключается к брокеру Sber каждые несколько секунд,
  ...
  тот же логин одновременно использовался где-то ещё, брокер отказывал в
  доступе к топикам или выкидывал клиента, мост бесконечно подключался и
...
- **Уведомление «Сущность не найдена» в разделе «Ремонт» теперь действительно
  ...
  уведомление, не срабатывала никогда. Теперь уведомление появляется, а
  исчезает, когда сущность вернулась или устройство убрали из моста.
...
- **Отклонённый пароль MQTT теперь виден, а не выглядит как «Нет связи».**
  ...
  интеграции с формой повторного входа; статус на вкладке «Статус» и в панели
  инструментов называет причину. Датчик «Фаза подключения», «Информация о
  системе» и диагностика получили значение `auth_failed` («Логин или пароль
  отклонён»). Если у вас есть автоматизации на фазу `disconnected`, добавьте в
