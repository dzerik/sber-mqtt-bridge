...
- **В шапке панели счётчик был подписан неверно.** Надпись «известно
  Сберу» показывала число устройств, подтверждённых в текущей сессии, —
  из-за чего шапка и таблица показывали разные числа для одного и того
  же. Теперь в шапке настоящее число известных облаку устройств.

  Заодно закрыт пробел в тестах: проверки перезапуска подменяли реестр
  известных устройств заглушкой, поэтому мост, ни разу его не
  заполнивший, прошёл бы их все. Добавлены тесты через настоящую
  публикацию конфигурации.

## [1.48.0] - 2026-09-01

### Added

- **Панель полностью переведена на русский.** Раньше переводились
  только настройки ворот и чайника, остальное было на английском.
  Теперь переведены все восемнадцать экранов: главный список устройств,
  мастер добавления, настройки, состояние подключения, статистика и все
  вкладки инструментов разработчика — 294 строки.

  Язык берётся из настроек Home Assistant тем же штатным механизмом,
  каким пользуется встроенная панель ZHA, поэтому английский интерфейс
  остаётся доступным без каких-либо действий.

### Fixed

- **Одно состояние подключения называлось по-разному** в шапке и в
  карточке — «Ready» против «Connected». Теперь везде одинаково.

- Поправлена типографика русских переводов: «MQTT Логин» → «Логин
  MQTT», «SSL сертификат» → «SSL-сертификат», «MQTT брокер Sber» →
  «MQTT-брокер Сбера» и ещё восемь мест.

## [1.47.1] - 2026-09-01

### Fixed

- **После перезапуска Home Assistant все устройства выглядели
  непринятыми** (issue #57). Панель показывала их как «не подтверждённые
  Сбером» до тех пор, пока пользователь не откроет приложение Салют, —
  хотя устройства всё это время оставались заведёнными в облаке.

  Отметка подтверждения ставится, когда Сбер сам обращается к
  устройству, и хранилась только в памяти, поэтому перезапуск её
  обнулял. Спросить облако напрямую нельзя: в протоколе нет запроса
  наверх, мост только публикует и слушает. Но список устройств, о
  которых облако спрашивало, сохраняется между перезапусками — просто
  панель его не показывала.

  Теперь в панели три состояния вместо двух: «известно Сберу»
  (сохраняется между перезапусками), «подтверждено в этой сессии» и
  «не подтверждено ни разу». Тревожное — только последнее: устройство,
  о котором облако не спрашивало никогда, это и есть признак
  молчаливого отклонения. Оценка состояния моста и предупреждение о
  молчаливом отклонении теперь опираются на него же и больше не
  срабатывают после каждой перезагрузки.

## [1.47.0] - 2026-08-26

### Fixed

- **Чайники не управлялись из приложения.** Мост отправлял им
  `water_heater.turn_on` — необязательный сервис Home Assistant,
  который умные чайники (SkyKettle и подобные) не реализуют. Команда
  температуры уходила отдельно и выставляла уставку, не запуская
  нагрев. Теперь мост управляет чайником через режимы работы, как это
  и задумано в Home Assistant: выключить, вскипятить, нагреть до
  заданной температуры. Имена режимов у каждой модели свои — мост
  распознаёт привычные автоматически, а если у вашего чайника они
  необычные, их можно выбрать вручную в настройках устройства из
  списка, который сообщает сам чайник. Чайники, подключённые как
  обычный выключатель, работают как прежде.

### Added

- **Таймер автозакрытия ворот.** Многие платы управления сами
  закрывают створку через заданное время. Мост об этой настройке знать
  не может, поэтому её можно указать вручную — и тогда в приложении
  видно, что ворота поехали закрываться. Отсчёт начинается по датчику
  положения, а не по нашей команде, поэтому работает и когда ворота
  открыли пультом или звонком на GSM-модуль. Показания датчика всегда
  главнее: как только он сообщает новое положение, догадка отменяется.

- **Проверка значений на соответствие словарю Sber.** Сборщик схемы
  теперь читает со страницы каждой функции полный перечень допустимых
  значений, а DevTools отмечает случаи, когда мост отправляет в облако
  значение, которого в этом перечне нет — облаку просто некуда такое
  значение направить. Эталон берётся именно со страницы функции, а не
  из примера на странице категории: примеры неполны, и сверка с ними
  забраковала бы исправные устройства.

  Проверка сразу нашла два расхождения. У **телевизора** источник
  сигнала уходит подписью из Home Assistant («HDMI 1») вместо
  протокольного `hdmi1`. У **пылесоса** программа уборки отправляется
  названиями скоростей вентилятора, а статус собирается из значений,
  которых в документации Sber нет вовсе. Обе правки меняют описание
  модели устройства, из-за чего облако перерегистрирует устройство и
  пользователь теряет назначенную комнату, — поэтому они вынесены
  отдельно и здесь не сделаны.

- **Проверка числовых значений по диапазонам.** Для функций с
  задокументированными границами (влажность 0–100, давление 200–800 и
  ещё 27) выход за пределы отмечается предупреждением: облако такое
  значение может обрезать или отбросить.

- **Проверка условно обязательных функций.** В документации Sber часть
  функций помечена «хотя бы одна из»: воротам нужен какой-нибудь способ
  открытия, датчику воздуха — хоть одно измерение. Мост эти пометки
  собирал, но до кода не доводил, поэтому устройство, не объявившее ни
  одной из группы, молча отклонялось облаком без всякой диагностики.
  Теперь такие случаи видны в DevTools как ошибка.

### Changed

- **Описание «Времени хода створки» переписано.** Пользовательская
  проверка на живых воротах показала: пока мост сообщает
  «открывается» или «закрывается», приложение Сбера блокирует кнопку
  управления в обе стороны на весь заданный таймер, и прервать
  движение из приложения нельзя. Размен «точный статус против
  возможности вмешаться» теперь описан прямо в настройке, а не
  обнаруживается на воротах.

## [1.46.0] - 2026-08-13

### Added

- **Ворота из пары «импульсное реле + геркон»** (issue #53). Гаражные
  приводы вроде Tuya TS0603 приходят в Home Assistant двумя сущностями:
  выключатель, замыкание которого запускает движение, и датчик положения.
  Собственного состояния у такого выключателя нет, поэтому раньше он мог
  стать только реле, а ворота в приложении Сбера не появлялись. Теперь
  выключатель (а также `button` и `script`) можно вручную назначить
  категорией «Ворота», связав с ним датчик положения в новой роли
  «Состояние створки» — мастер добавления предлагает такую пару сам.
  Положение берётся исключительно от датчика: состояние самого реле
  залипает и о воротах не говорит ничего.

  Команда «открыть» на уже открытых воротах намеренно не отправляет
  импульс: у привода одна кнопка на оба направления, и без такой защиты
  голосовая команда закрыла бы створку — возможно, на стоящую под ней
  машину. Подряд идущие команды объединяются в один импульс (окно 2 с).

  В настройках сущности появились два параметра: инверсия датчика (если
  геркон сообщает «включено» на закрытых воротах) и способ подачи
  импульса — по умолчанию `toggle`, потому что импульс возникает на
  *смене* значения, а не на повторной записи того же.

  Команда «стоп» не объявляется: поведение импульса на середине хода не
  подтверждено на живом устройстве, а объявленный режим дал бы голосовую
  команду, которая молча ничего не делает.

- **Время хода створки** — необязательная настройка ворот. Без неё (так
  по умолчанию) приложение показывает новое положение только после
  срабатывания датчика, а у медленных ворот это десятки секунд. Если
  указать время хода, мост сразу после импульса сообщает «открывается»
  или «закрывается», а срабатывание датчика немедленно отменяет догадку
  и заменяет её фактом. Если за отведённое время датчик так и не
  сработал, мост возвращает прежнее положение и пишет предупреждение —
  выдумывать, что створка доехала, он не станет.

- **Предупреждение о воротах без датчика положения.** Категорию можно
  назначить и вручную, минуя мастер, и тогда ворота остались бы молча
  сообщать «закрыто» при любом положении створки. Теперь такая пара
  видна в разделе «Ремонт» Home Assistant, в диагностике и в карточке
  устройства; предупреждение снимается само, как только датчик привязан.

- **Панель говорит по-русски.** Раньше переводы существовали, но панель
  их не читала, и все подписи были английскими. Теперь она берёт строки
  из файлов перевода интеграции, как это делает встроенная панель ZHA.

### Fixed

- **Бойлер не принимал ни одну команду** (issue #33). Устройства
  категории `hvac_boiler` живут в домене `water_heater`, но мост слал им
  команды в домен `climate` — то есть вызывал несуществующий сервис, и
  управление из приложения не работало вовсе. Домен сервиса теперь
  всегда берётся у самой сущности; это же чинит любое устройство,
  которому пользователь вручную назначил категорию из другого домена
  (шторы, ворота, свет, вентиляция, увлажнитель, пылесос, телевизор).

- **Сохранение настроек ворот перезагружало всю интеграцию** — из-за
  одного переключателя кратковременно рвалась связь с облаком. Теперь
  настройка применяется на месте.

## [1.45.1] - 2026-08-11

### Fixed

- **Переопределение класса устройства из интерфейса Home Assistant
  игнорировалось** (issues #50, #51). HA хранит два значения: то, что
  сообщила интеграция (`original_device_class`), и выбор пользователя в
  настройках сущности — «Показывать как» (`device_class`). Мост читал
  только первое, поэтому пользователь, пометивший привод как «Гараж»,
  всё равно видел устройство в категории штор. Теперь используется
  действующее значение, как и везде в Home Assistant. Это же делает
  рабочим штатный путь для реле ворот: помощник «Switch as X» превращает
  выключатель в полноценный `cover`, а «Показывать как: Гараж»
  отправляет его в категорию `gate`.
- **Одинаковая конфигурация публиковалась по несколько раз подряд.** При
  старте совпадали несколько триггеров, и в облако уходили до трёх
  идентичных пакетов `up/config`. Публикация пропускается, если payload
  побайтово совпадает с предыдущим; явный «Re-publish» из панели и запрос
  от Sber отправляют всегда.
- **Идентификатор модели «плавал» между перезапусками.** У датчика,
  чья влажность приходит от связанной сущности, набор возможностей
  зависел от того, успела ли та отчитаться к моменту публикации: один и
  тот же физический датчик получал то `Mdl_sensor_temp_594cb2b1`, то
  `Mdl_sensor_temp_24b4919e`. Готовность теперь учитывает связанные
  сущности, поэтому конфигурация публикуется с полным набором фич.

## [1.45.0] - 2026-08-11

### Fixed

- **Спящие Zigbee-датчики всё ещё теряли комнату** (issue #44). Батарейные
  датчики просыпаются позже окна затишья, поэтому первая публикация уходила
  без них, а вторая — уже с ними, и облако считало их новыми. Подкрутка
  таймаутов лечила симптом: длительность сна заранее неизвестна.

  Вместо угадывания времени введён инвариант: **конфигурация никогда не
  публикуется без устройства, которое облако уже держит**. Мост ведёт
  реестр известных облаку устройств — пополняет его из собственных
  успешных публикаций и из списка, который Sber присылает в каждом
  `status_request`, — и сохраняет между перезапусками. Ждём теперь не
  «всех», а только тех, чьё отсутствие разрушительно: при первой установке
  медленный датчик никого не задерживает, а после перезапуска устройство,
  известное облаку, задержит публикацию до своего пробуждения.

  Прочитать состояние у облака напрямую нельзя — протокол односторонний
  для дескрипторов устройств, и `partner_meta` обратно не возвращается;
  `status_request` оказался единственным доступным источником этой
  информации, и его достаточно.

  Окно затишья по-прежнему коалесцирует пачку, а потолок остаётся
  аварийным выходом: если известное облаку устройство исчезло из HA
  насовсем, публикация уходит без него с предупреждением.

### Added

- **Настройки таймингов доступны из интерфейса.** Раньше `config_settle_delay`
  и `config_max_wait` (как и `confirm_delay` с `ack_audit_delay`) не были
  выведены никуда и задавались только правкой файла настроек. Теперь они
  есть и на вкладке настроек панели, и в Home Assistant:
  Настройки → Устройства и службы → Настроить → Тайминги синхронизации
  устройств.
...
- **Комнаты устройств сбрасывались к комнате хаба при каждом рестарте
  Home Assistant** (issue #44). Sber считает каждое сообщение `up/config`
  полным и авторитетным списком устройств. Home Assistant же поднимает
  сущности постепенно — Zigbee-координатору нужны десятки секунд, — а
  сборщик конфигурации пропускает сущности, ещё не получившие состояние.
  Публикация происходила на **каждую** ставшую доступной сущность, то есть
  облако получало серию неполных списков: отсутствующие устройства оно
  считало удалёнными, а при появлении в следующем пакете — новыми, и
  назначало им комнату хаба, игнорируя переданное поле `room`. Ручная
  публикация из панели работала именно потому, что уходила одним пакетом.

  Добавлен коалесцирующий шлюз `ConfigPublishGate` с тремя ярусами:
  публикация сразу, если все сущности уже готовы; окно затишья, которое
  перевзводится на каждую новую сущность; и жёсткий потолок, после которого
  конфигурация уходит без так и не поднявшихся сущностей — с
  предупреждением, перечисляющим их поимённо. Тот же шлюз теперь
  придерживает и публикацию при подключении к брокеру: раньше она могла
  уйти неполной, если MQTT подключался раньше, чем прогружались устройства.

### Added

- Настройки `config_settle_delay` (окно затишья, по умолчанию 5 с) и
  `config_max_wait` (потолок ожидания, по умолчанию 120 с). Значения
  подобраны под Zigbee/Z-Wave: 5 с перекрывают паузы внутри пачки
  устройств, 120 с — подъём крупной сети, оставаясь ограничением для
  стика, который не поднялся вовсе.

## [1.43.2] - 2026-08-09

### Fixed

- **Вкладка Raw Config показывала `parent_id: "root"` независимо от
  настройки** (issue #44). Предпросмотр вызывал сборщик конфигурации, не
  передавая ему ни `auto_parent_id`, ни `default_home` / `default_room`, и
  молча получал дефолты самой функции. В результате пользователь, выключив
  тумблер «Auto-assign parent_id», по-прежнему видел иерархию у всех
  устройств и делал вывод, что настройка не работает. Публикуемая
  конфигурация при этом была корректной — врал именно предпросмотр,
  созданный ровно для того, чтобы показывать «что уйдёт в облако».
  Теперь предпросмотр использует тот же объект контекста, что и реальная
  публикация, поэтому разойтись они больше не могут; равенство закреплено
  тестом на обоих положениях тумблера.
- Дефолт `auto_parent_id` у `build_devices_list_json` изменён с `True` на
  `False` — теперь он совпадает с `SETTINGS_DEFAULTS`. Забытый аргумент
  больше не включает иерархию, которую пользователь выключил.
...
`docs/reviews/2026-08-07-full-code-review.md`, 110 подтверждённых находок,
4 critical) и пяти волн ремедиации, плюс разбор повторного отчёта по
issue #44. Каждая правка прошла независимое adversarial-ревью с
мутационными пробами.

### Fixed — многоканальные устройства (issue #44)

- **Состояния публиковались вне заявленных возможностей.** Лампа с
  `supported_color_modes: ["onoff"]` объявляла `[online, on_off]`, но
  публиковала `light_brightness = 100` (и `light_mode` во включённом
  состоянии): при отсутствии HA-атрибута `brightness` конвертер выдавал
  минимум шкалы Sber, а условие `!= 0` его пропускало. Собственный
  schema-валидатор моста фиксировал это как `not_declared`. Фильтрация
  состояний по итоговому списку фич (с учётом `sber_features_add` /
  `sber_features_remove`) сделана один раз в `BaseEntity`, поэтому
  закрыт весь класс утечек во всех 28 категориях; `online` проходит
  всегда — он обязателен в каждой категории спеки. Попутно нашлись и
  устранены такие же утечки: `source` у TV без `source_list`,
  `vacuum_cleaner_program` у пылесоса без `fan_speed_list`.
- **Sber склеивал каналы одного устройства.** Обе сущности
  многоканальной люстры получали одинаковый `model_id`
  (`{device.model_id}_{category}`) при разных наборах фич, а облако
  хранит одну модель на id — второй контур отображался как цветная лампа
  с неработающими ползунками. Теперь в `model_id` добавляется дайджест
  набора возможностей (features + allowed_values). Он детерминирован
  между перезапусками, не зависит от порядка загрузки и не привязан к
  `entity_id`: две одинаковые лампы по-прежнему делят одну модель, а два
  контура одной люстры — нет. **Эффект обновления:** `model_id` меняется
  у всех устройств, облако перерегистрирует модели при первой публикации
  конфигурации; идентификаторы устройств не меняются, повторное
  сопряжение не требуется.
- **Панель обрезала JSON без визуального признака.** Блоки payload имели
  `max-height` со скрытой областью прокрутки: на мобильном JSON выглядел
  оборванным (не хватало закрывающих скобок), из-за чего пользователь
  сообщил о «невалидном JSON», которого не было. Введён общий
  раскрывающийся компонент `sber-json-block`: показывает срез с честным
  счётчиком скрытых строк, раскрывается с клавиатуры (`aria-expanded`,
  `aria-controls`), копирует **полный** payload независимо от свёрнутости.

### Security

- **Ни одна из 41 WebSocket-команды не требовала прав администратора.**
  Любой аутентифицированный не-админ HA мог публиковать произвольный JSON
  в облако Sber, отключить проверку TLS-сертификата, стереть конфигурацию,
  читать весь MQTT-трафик, а через `inject_sber_message` — вызывать
  сервисы HA с безличным `Context()`, то есть в обход per-user entity
  permissions. `require_admin` навешен в единой точке регистрации, поэтому
  новая команда защищена автоматически; панель тоже admin-only.
- **Диагностика раскрывала логин Sber.** Он же MQTT-username и корневой
  сегмент топика (`sberdevices/v1/<login>/…`), а файлы диагностики
  штатно прикладывают к публичным issue — теперь редактируется.
- `update_settings` и `ws_import` писали непроверенные значения в
  `entry.options`: битый импорт ломал setup интеграции без возможности
  починки через UI. Добавлены строгие схемы.
- Входящий MQTT-payload проверяется по размеру **до** декодирования и
  записи в кольцевые буферы DevTools (удалённая сторона могла держать
  десятки МБ), сохраняемая копия усечена до 8 КБ.

### Fixed

- **Одно некорректное сообщение от Sber навсегда убивало MQTT-соединение.**
  `run()` ловил лишь узкий набор исключений, поэтому `AttributeError` из
  обработчика выходил из цикла и мост молча уходил в офлайн до перезапуска
  HA. Добавлен барьер вокруг обработки одного сообщения (отмена
  пробрасывается), состояние сбрасывается в `finally`.
- **Type-confusion во входящем payload.** `[1,2,3]`, не-dict `devices`
  или `states` давали `AttributeError`; добавлена структурная санитизация,
  включая `RecursionError` на глубоко вложенном JSON.
- **Lost update при публикации состояний.** Снимок «опубликовано»
  делался после `await publish` и фиксировал уже новое состояние —
  изменение, случившееся во время отправки, считалось доставленным и
  терялось навсегда. Снимок берётся из фактически отправленного payload.
  Инвариант «сломанная сущность не блокирует весь батч» закреплён тестом.
- **Смена `verify_ssl` из панели не применялась никогда:** SSL-контекст
  создавался один раз до цикла реконнекта.
- **Stale-таймер прошлого подключения** досрочно снимал ack-guard нового
  (`activate()` не отменял предыдущий), а debounce-таймер персиста
  redefinitions переживал unload и затирал настройки преемника.
- **Ошибка одной сущности роняла загрузку всех** и весь
  `async_setup_entry`; сущность с `device_id` без `linked_device`
  оставалась в реестре и позже валила публикацию конфигурации целиком.
- **Падение одного устройства прерывало весь батч команд**, включая ack.
- **Тесты config_flow были исключены из CI** (`-k "not test_config_flow"`)
  и падали локально: HA-компонент `frontend` требует пакет
  `hass_frontend`, который не тянется транзитивно. Устранена
  первопричина, исключение убрано.
- **Пять DevTools-компонентов панели навсегда теряли WebSocket-подписку**
  после переключения вкладок (одноразовый guard + отписка в
  `disconnectedCallback`). Подтверждено браузерной пробой до/после.
- JS-валидатор имён расходился с Python: запрещал дефис и блокировал
  wizard. Приведён к побайтовому соответствию; сама проверка стала
  совещательной, как и на бэкенде.
- Слепые `setTimeout(1500)` после мутаций в панели заменены опросом
  состояния с таймаутом — пользователь больше не видит ложный успех.
- Многократное добавление в wizard стало атомарным: частичный сбой
  сообщает, какие устройства не добавились, повтор не дублирует links.

### Changed

- **Границы компонентов стали настоящими.** `SberPublisher`,
  `SberCommandDispatcher` и `RedefinitionsStore` получали в конструктор
  весь `SberBridge` и читали ~25 его приватных полей (~100 обращений).
  Введены узкие зависимости (`PublisherDeps`, `DispatcherDeps`,
  `StatsPort` в новом `bridge_ports.py`) по образцу `HaStateForwarder`;
  `BridgeCommandContext`, состоявший из приватных членов, удалён. Ни один
  коллаборант больше не импортирует `sber_bridge` — закреплено
  AST-тестом. Из `SberBridge` убраны backward-compat прокси, включая
  сеттеры в приватные поля чужих объектов.
- Единый источник истины для реестров: `ALL_LINKABLE_ROLES` собирается из
  `LINKABLE_ROLES` device-классов (air-роли больше не теряются в wizard),
  `OVERRIDABLE_CATEGORIES` выводится из `CATEGORY_DOMAIN_MAP` (было три
  расходящихся определения: 21 / 28 / 21+auto в JS), `create_ssl_context`
  живёт только в `ssl_utils`.
- Базовая доступность панели с клавиатуры: `role`/`tabindex`/`keydown`,
  `aria-label`, focus-visible, модальность и возврат фокуса в диалогах
  (ранее во всём `www/` не было ни одного `aria-*`/`role`/`tabindex`).
- Coverage-гейт `fail_under` поднят с 50 % до 85 % при фактическом
  покрытии 96,5 % (CLAUDE.md декларировал минимум 80 %).

### Removed

- Мёртвый код: `RECONNECT_INTERVAL_MIN/MAX`, `MAX_MQTT_PAYLOAD_SIZE`,
  `BridgeStats.last_ack_time`, дубликат `_reconnect_interval`,
  `_sweep_traces`, enum `HAState`, `ack_audit.audit_delay`,
  `DIALOG_STYLES_CSS`, `filterEntities`, каталог `tests/hacs/snapshots`
  (syrupy 5.x читает `__snapshots__`).
