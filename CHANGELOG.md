...
- **Проверка типов (mypy) теперь обязательна в CI.** Раньше она была описана
  в инструкции для разработчиков, но нигде не запускалась — и запуститься не
  могла: настройка указывала Python 3.13, а Home Assistant 2026.4+ использует
  синтаксис 3.14. Теперь CI проверяет весь код моста и не пропускает новые
...
- **Сборка релизов защищена от подмены сторонних действий GitHub.** Все
  ...
  выпускаются только из ветки, их ревизию обновляем вручную. Права на запись в репозиторий получают только
  шаги, которым они действительно нужны: публикация релиза, документации и
  PR о смене документации Sber; проверки и тесты работают только на чтение.
