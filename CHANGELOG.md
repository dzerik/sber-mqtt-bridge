...
- **Ворота из пары «импульсное реле + геркон»** (issue #53). Гаражные
  ...
  *смене* значения, а не на повторной записи того же.
  ...
  Ограничения этого выпуска: команда «стоп» не объявляется (поведение
  импульса на ходу не подтверждено на живом устройстве), промежуточные
  состояния «открывается» / «закрывается» не эмулируются — приложение
  показывает новое положение после срабатывания датчика. Так же ведёт
  себя штатная интеграция Tuya в Home Assistant.

### Changed

- Команды штор и ворот вызываются в домене самой сущности, а не в жёстко
  прописанном `cover` (issue #33, частично).
