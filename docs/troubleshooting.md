...
| Проблема | Решение |
|----------|---------|
| Не удаётся подключиться | Проверьте учётные данные в Sber Studio. Убедитесь, что проект активен. При запуске интеграция проверяет подключение к брокеру: если брокер недоступен, запись остаётся в состоянии повторной настройки, и Home Assistant сам повторяет попытку; если брокер отклонил пароль, появляется запрос повторной аутентификации. |
...
| После перезапуска HA устройства переезжают в комнату хаба | Часть устройств не успела загрузиться до публикации списка, и Сбер завёл их заново. Увеличьте «Окно затишья» и «Максимальное ожидание устройств» — см. [Настройка](configuration.md); имена не загрузившихся устройств — в предупреждении журнала. |
...
Потеря связи с брокером записывается в журнал одним предупреждением, а возврат — одним информационным сообщением «restored». Неудачные попытки между ними видны только в отладочных логах.
